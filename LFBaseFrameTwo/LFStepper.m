...
- (void)commonInit{
    ...
    //显示数据的text
    self.countText = [[UITextField alloc] init];
    self.countText.textAlignment = NSTextAlignmentCenter;
    self.countText.layer.borderWidth = 1.0f;
    [self.countText addTarget:self action:@selector(changeValueTapped:) forControlEvents:UIControlEventEditingDidEnd];
    self.countText.keyboardType = UIKeyboardTypeNumberPad;
...
- (void)layoutSubviews
{
    ...
    self.countText.frame = CGRectMake(self.buttonWidth, 0, width - (self.buttonWidth * 2), height);
...
- (CGSize)sizeThatFits:(CGSize)size
{
    if (CGSizeEqualToSize(size, CGSizeZero))
    {
        ...
        CGSize labelSize = [self.countText sizeThatFits:size];
...
- (void)setBorderColor:(UIColor *)color
{
    ...
    self.countText.layer.borderColor = color.CGColor;
}
...
{
    self.countText.textColor = color;
}

- (void)setLabelFont:(UIFont *)font
{
    self.countText.font = font;
}
...
- (void)changeValueTapped:(id)sender
{
    UITextField *textStepper = (UITextField *)sender;
    
    if (! [self isPureInt:textStepper.text]) {
        //不是整型，默认给最小值
        textStepper.text = [NSString stringWithFormat:@"%@",@(self.minimum)];
    }
    
    float textValue = [textStepper.text floatValue];
    
    if (textValue < self.minimum) {
        self.value = self.minimum;
        //        if (self.valueChangedCallback){
        //            self.valueChangedCallback(self, self.minimum);
        //        }
    } else if (textValue > self.maximum) {
        self.value = self.maximum;
        //        if (self.valueChangedCallback){
        //            self.valueChangedCallback(self, self.maximum);
        //        }
    } else {
        self.value = textValue;
        //        if (self.valueChangedCallback){
        //            self.valueChangedCallback(self, textValue);
        //        }
    }
    
    //上面的注释有必要，因为在对value的setting方法中会调用一次valueChangedCallback这个block
    //这样再调用一次self.valueChangedCallback方法会重复调用valueChangedCallback这个block
